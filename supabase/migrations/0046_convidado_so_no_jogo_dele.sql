-- ============================================================
-- 0046: convidado vale para UM jogo.
-- ============================================================
-- Como o convidado vira membro do grupo, ele aparecia em todos os jogos
-- seguintes na lista de quem nao respondeu - 33 nomes de semanas passadas
-- poluindo a lista da semana.
--
-- Agora o convidado so aparece no jogo em que tem resposta, ou seja, naquele em
-- que foi convidado. Ele NAO e apagado: continua no grupo, no historico e no
-- financeiro. Some apenas das listas dos jogos que nao sao dele.
--
-- Para voltar atras: as versoes anteriores destas funcoes estao nas migrations
-- 0032 (public_attendance_list) e 0038 (public_tournament_summary).
...
begin
  ...
  from (
    ...
    where gm.group_id = t.group_id and gm.status = 'active'
      -- Convidado de outra semana nao entra na lista desta.
      and (not gm.is_guest or a.id is not null)
  ) x;
...
begin
  ...
  from (
    ...
    left join public.attendance a
      on a.member_id = gm.id and a.tournament_id = t.id
    where gm.group_id = t.group_id and gm.status = 'active'
      -- Convidado de outra semana nao entra no resumo desta.
      and (not gm.is_guest or a.id is not null)
  ) x;
