...
export async function createGroup(formData: FormData) {
  ...
  const modality = String(formData.get("modality") || "beach");
  if (!name) return;

  const { data, error } = await supabase
    .from("groups")
    .insert({ name, description, color, modality, owner_id: user.id })
    .select("id")
    .single();
...
export async function updateGroup(groupId: string, formData: FormData) {
  ...
  const { error } = await supabase
    .from("groups")
    .update({ name, description, color, modality })
    .eq("id", groupId);
  if (error) throw new Error(error.message);
...
export async function updateSettings(groupId: string, formData: FormData) {
  ...
  const guest_fee = guestRaw ? Number(guestRaw) || null : null;
